...
import { ButtonGroup, Button, HorizontalGroup, InlineField, InlineLabel, Input, LoadingPlaceholder, stylesFactory, useTheme, IconButton } from "@grafana/ui";
...
export const WritePointValueModal = (props: any) => {
  ...
  const onChange = (value: number | null, priorityKey: string, reset: boolean = false,) => {
  ...
  return <ThemeProvider theme={materialTheme}>
        ...
        {Object.keys(formData).map((priorityKey: string) => (
          <Grid item xs={6} sm={3} key={priorityKey} >
            <InlineField labelWidth={6} label={getKeyLabel(priorityKey)}>
              <Input placeholder={priorityKey}
                key={keys[priorityKey]}
                type="number"
                defaultValue={formData[priorityKey]}
                onChange={function (v) {
                  // @ts-ignore
                  return onChange(parseInt(v.nativeEvent.target?.value), priorityKey);
                }}
                disabled={
                  priorityKey !== "_7" &&
                  priorityKey !== "_14" &&
    ...
    {confirmLoading && <div className={styles.overlayRunning} > <LoadingPlaceholder /></div>}
  </ThemeProvider>
};
